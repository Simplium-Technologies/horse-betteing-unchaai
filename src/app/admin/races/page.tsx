...
interface Race {
  ...
  seasonId: string | null; season: Season | null;
...
export default function AdminRacesPage() {
  ...
  async function fetchData() {
    const [racesRes, horsesRes, seasonsRes] = await Promise.all([
    ...
    if (seasonsData.success) setSeasons(seasonsData.seasons.filter((s: Season) => s.status !== "COMPLETED"));
    setLoading(false);
  }
  ...
  function resetForm() {
    setFormName(""); setFormDuration(""); setFormAutoClose(false); setFormIncludePoints(true); setFormHorseIds([]); setFormSeasonId("");
  ...
  async function handleCreate() {
    ...
    try {
      ...
      const body = isEditing
        ? { id: editingRace.id, name: formName, durationMinutes: Number(formDuration), autoClose: formAutoClose, includePoints: formIncludePoints, horseIds: formHorseIds, seasonId: formSeasonId || null }
        : { name: formName, durationMinutes: Number(formDuration), autoClose: formAutoClose, includePoints: formIncludePoints, horseIds: formHorseIds, seasonId: formSeasonId || null };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-4 sm:mb-6">
        <h2 className="text-lg sm:text-xl font-bold text-gray-900 tracking-tight">Races</h2>
        <Button onClick={() => { resetForm(); setShowForm(true); }}>+ Create Race</Button>
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-2xl card-shadow border border-gray-100 p-4 sm:p-6 mb-4 sm:mb-6 animate-slide-up">
          ...
          <div className="space-y-3 sm:space-y-4 max-w-lg">
            ...
            <div>
              <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-2">Season (optional)</label>
              <select
                value={formSeasonId}
                onChange={(e) => setFormSeasonId(e.target.value)}
                className="w-full min-h-[44px] px-4 border border-gray-200 rounded-xl outline-none focus:border-[#17251c] text-gray-900 text-sm bg-white"
              >
            ...
            <div className="flex gap-2 sm:gap-3">
              <Button onClick={handleCreate} loading={saving}>{editingRace ? "Save Changes" : "Create Race"}</Button>
      ...
      {races.length === 0 ? (
        <EmptyState icon="🏁" title="No races yet" description="Create your first race to get started." />
      ) : (
              ...
              <div key={race.id} className="bg-white rounded-xl sm:rounded-2xl card-shadow border border-gray-100 p-3 sm:p-5 hover:card-shadow-hover transition-all">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 sm:gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    {race.season && (
                      <p className="text-[10px] sm:text-xs text-[#c9a84c] font-semibold mt-0.5">{race.season.name}</p>
                    )}
